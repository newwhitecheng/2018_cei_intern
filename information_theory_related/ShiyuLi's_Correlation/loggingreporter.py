    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for lndx, layerix in enumerate(self.layerixs):
            ...
            if self.cfg['FULL_MI']:
                ...
                data['activity_tst'].append(self.layerfuncs[lndx]([self.tst.X[:2000, :, :, :],])[0])
...
class weightLogging(keras.callbacks.Callback):
    def __init__(self, cfg, do_save_func=None, *kargs, **kwargs):
        # do_save_func(epoch) should return True if we should save on that epoch
        super(weightLogging, self).__init__()
        self.do_save_func = do_save_func
        self.cfg = cfg
