...
lines=""
manager = Manager()
finished = manager.Value('finished',0)
MAX_GPUS = 1

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#test_path = '/hd44/caffe/caffe-xinbai/TT/py-R-FCN-imagenet/data/demo/4001'
...
caffemodel = os.path.join(cfg.DATA_DIR, 'rfcn_models','resnet50_rfcn_ohem_iter_319.caffemodel')
...
def vis_detections(im, class_name, dets, image_name, data, thresh=0.4):
    ...
    for i in inds:
        ...
        data['image_name'].append(ID)
        data['CLASS_ID'].append(class_name)
        data['confidence'].append(score)
        data['xmin'].append(bbox[0])
        data['ymin'].append(bbox[1])
	data['xmax'].append(bbox[2])       
	data['ymax'].append(bbox[3]) 
	#print(os.path.splitext("path_to_file")[0])
        #text_file = open("answer0707.csv", "a")
        #text_file.write(z)
        #text_file.close()
...
def demo(net, im_file):
    ...
    #timer = Timer()
    #timer.tic()
    #print datetime.now(),"begine calling im_detect(%s)" %im_file
    scores, boxes = im_detect(net, im)
    #print datetime.now,"end im_detect(%s)"%im_file
    #timer.toc()
...
#0.1 -->  #0.2->  #0.01 ->0.315
    NMS_THRESH = 0.3
    data = {'image_name': [], 'CLASS_ID': [],'confidence': [],
               'ymax': [],  'xmax': [],
               'xmin': [], 'ymin': []}
    ...
    for cls_ind, cls in enumerate(CLASSES[1:]):
        ...
        dets = dets[keep, :]
        #print datetime.now(),"begine vis_detections(%s)"% image_name
        vis_detections(im, cls, dets, thresh=CONF_THRESH, image_name=image_name,data=data)
        #print datetime.now(),"end vis_detections(%s)"% image_name
    im = None
...
def test_reference(im_path, token,net):
    print os.getpid(), datetime.now(), "start demo", im_path  
    res = demo(net, im_path)
    if len(res['image_name']) > 0:
        if post_result(token,res)==0:
            print "Posting Result Failed, Exiting", im_path 
            sys.stdout.flush()
            sys.exit()
        else:
            #print "success post_result for", im_path        
            pass
        print os.getpid(), datetime.now(), "checked", im_path,"objects",len(res['image_name'])
    print os.getpid(), datetime.now(), "finished demo", im_path 
    ...
    while True:
        try:   
            ...
            if not get_images(token,w):
                ...
                unzip_images(w)
                fpath='../images/%s'%w
                for im_name in os.listdir(fpath): 
                    im_path = os.path.join(fpath,im_name)
                    q.put(im_path)
        except:
            traceback.print_exc()
            break
...
def producer_finished(result):
    global finished
    print datetime.now(), "producer finished"
    finished.value = 1


def consumer(q, namespace,gpuid):
    global finished
    global token
    #global net
    #net = caffe.Net(prototxt, caffemodel, caffe.TEST)
    try:
        print datetime.now(), os.getpid(), 'consumer start, gpuid:',gpuid
        caffe.set_mode_gpu()
        '''
        if gpuid < MAX_GPUS:
        ...
        '''
        net = caffe.Net(prototxt, caffemodel, caffe.TEST)
        print datetime.now(), os.getpid(), 'consumer allocate gpu on:',gpuid
    except:
        print datetime.now(), os.getpid(), 'consumer net error, gpuid:',gpuid
        traceback.print_exc()
        return None
    ...
    while True:
        try:
            if q.qsize() == 0 and finished.value == 1: 
                print os.getpid(),"qsize == 0 break"
                sys.stdout.flush()
                break
            if q.qsize() == 0: 
               time.sleep(2.0)
            ...
            w = q.get(False)
            try:
                test_reference(w, token,net)
                sys.stdout.flush()
                #print 'get w',w
            except:
                print "test_reference failed"
                traceback.print_exc()
                q.put(w,False)
                break
...
def multi_run(producer,consumer,consumer_cnt=1):
    global token
    global finished
    global manager
    ...
    print "ws",ws
    #ws = [i for i in range(101, 200+1, 100)]
    for i in ws: q1.put(i)
    print 'q1.qsize', q1.qsize()
  

    q = manager.Queue(10000)
    p = Pool(consumer_cnt + 1)
    nm = manager.Namespace()
    nm.running = True
    for i in xrange(4):
        pw = p.apply_async(producer,args=(q1,q,nm,),callback=producer_finished)
        print datetime.now(),'new producer',i
        sys.stdout.flush()
        time.sleep(0.1)
    for i in xrange(consumer_cnt):
        p.apply_async(consumer,args=(q,nm,i,))
        print datetime.now(),'new consumer',i
        sys.stdout.flush()
    p.close()
    p.join()
...
def get_images(token, image_number):
    ...
    if os.path.exists(image_directory+'/'+str(image_number)+'.zip'):
        return 1
...
#host_ipaddress = '10.30.5.243'
host_ipaddress = '192.168.1.7'
host_port = '8000'
password = 'pass@#2249'
score = 100
username = 'user_330'
csv_filename = 'answer0707.csv' #golden_output.csv'
...
deploy=root + 'examples/DR_grade/deploy.prototxt'    #deploy 
caffe_model=root + 'models/DR/model1/DRnet_iter_40000.caffemodel'  #caffemodel 
...
imgs = 1   #False     # Set to False if you want to use get_image
...
if len(sys.argv) >= 2 and sys.argv[1] == 'single':
    test_reference('../images/1/2190.jpg',token,net)
    sys.exit(0)
if camera_imgs:
    ...
    if imgs:
       multi_run(producer,consumer,consumer_cnt=2)
