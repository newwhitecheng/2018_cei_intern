...
caffemodel = os.path.join(cfg.DATA_DIR, 'rfcn_models','resnet50_rfcn_ohem_iter_100001.caffemodel')
...
def vis_detections(im, class_name, dets, image_name, data, thresh=0.4):
    ...
    for i in inds:
        ...
        data['ymax'].append(bbox[0])
        data['xmax'].append(bbox[1])
        data['xmin'].append(bbox[2])
        data['ymin'].append(bbox[3])
...
def consumer(q, namespace):
    global finished
    global token
    #global net
    net = caffe.Net(prototxt, caffemodel, caffe.TEST)

    #print datetime.now(), os.getpid(), 'consumer start'
    #net = caffe.Net(prototxt, caffemodel, caffe.TEST)
    ...
    while True:
        try:
            if q.qsize() == 0 and finished.value == 1: break
            if q.qsize() == 0: 
               time.sleep(2.0)
               continue
            w = q.get(False)
...
def multi_run(producer,consumer,pool_cnt=20,producer_cnt=10,consumer_cnt=10):
    ...
    for i in xrange(consumer_cnt):
        p.apply_async(consumer,args=(q,nm,))
...
if camera_imgs:
    ...
    if imgs:
       multi_run(producer,consumer,pool_cnt=10,producer_cnt=1,consumer_cnt=7)
