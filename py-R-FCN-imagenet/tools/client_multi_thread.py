...
def test_and_post(w):
    ...
    unzip_images(w)
    #for i in xrange(100):
    #    answer.csv=test_reference(w+i)   #result =  test_reference(w+i)
    read_csv(csv_filename)
    #read_csv(answer.csv) 
    line = get_lines(1)
    post_result(token,line)   #post_result(token,result)

def do_work(token,threadno,ws):
...
if camera_imgs:
    ...
    if imgs:
        ...
        THREADS = 10
        print 'no_of_images_packets', ws
        length = len(ws)
        each_size = length / THREADS
        if length % THREADS != 0: each_size += 1 
        start = 0
        for i in xrange(THREADS):
            thread.start_new_thread(do_work,(token,i,ws[i* each_size: min(length,(i+1)*each_size)]))
