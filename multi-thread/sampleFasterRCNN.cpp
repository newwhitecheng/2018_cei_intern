...
const std::string CLASSES[OUTPUT_CLS_SIZE]{ "background",
...
{
	std::vector<std::string> dirs{ "data/samples/faster-rcnn/", "data/faster-rcnn/","/home/nvidia/Images_2018/","/home/nvidia/workspace/ILSVRC2013_DET_val2/","/home/public/ILSVRC14/ILSVRC2013_DET_val2/" };
	return locateFile(input, dirs);
}

void jpg2ppm(const std::string& filename, PPM& ppm, int label)
{
...
pthread_mutex_t gpu_mtx = PTHREAD_MUTEX_INITIALIZER;
int cnt = 1;

void procedure(int pn)
{
	...
	//printf("time %f\n", duration);
		...
		for (int i = 0; i < N; ++i)
		{
			...
			jpg2ppm(imageList[i + pn], ppms[i],i+pn+1);
		...
		for (int i = 0; i < N; ++i)
		{
			...
			for (int c = 1; c < OUTPUT_CLS_SIZE; ++c) // skip the background
			{
				...
				{
					int idx = indices[k];
					//std::string storeName = CLASSES[c] + "-" + std::to_string(scores[idx*OUTPUT_CLS_SIZE + c]) + ".jpg";
					...
					std::cout << ppms[i].fileName<<" "<<c << " " <<CLASSES[c] << " " << scores[idx*OUTPUT_CLS_SIZE + c] << " " << b.x1*ppms[i].w / 224 << " " << b.y1*ppms[i].h / 224 << " " << b.x2*ppms[i].w / 224 << " " << b.y2*ppms[i].h / 224 << std::endl;
					//writePPMFileWithBBox(storeName, ppms[i], b);
					std::ofstream result_file;
					result_file.open("submission.csv", std::ios_base::app);
					result_file << ppms[i].label << "," << c << "," << scores[idx*OUTPUT_CLS_SIZE + c]
...
int main(int argc, char** argv)
{
	...
	caffeToGIEModel("faster_rcnn_test_iplugin.prototxt",
		...
		"vgg16_train_iter_300000.caffemodel",
	...
	imageList = getFiles("/home/nvidia/Images_2018/");
	//imageList = getFiles("/home/nvidia/Images_2018/");
	...
	//cpu_set_t mask[pnum/2];
	start = clock();
	for (int i = 0; i < pnum; i++)
    {
		//CPU_ZERO(&mask[i]);
        //CPU_SET(i, &mask[i]);
        pthread_create(&thread[i], NULL, mainthread, NULL);
		//pthread_create(&thread[i + pnum/2], NULL, mainthread, NULL);
		//sched_setaffinity(thread[i], sizeof(mask[i]), &mask[i]);
		//sched_setaffinity(thread[i + pnum/2], sizeof(mask[i]), &mask[i]);
    }
	...
	finish = clock();
	cout<<"using "<<(finish - start)/CLOCKS_PER_SEC<<" seconds"<<endl;
	context->destroy();
	engine->destroy();
	runtime->destroy();
	pluginFactory.destroyPlugin();
